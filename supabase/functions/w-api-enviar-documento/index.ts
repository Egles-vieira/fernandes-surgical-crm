...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('📤 Enviando documento via W-API, mensagemId:', mensagemId);
    ...
    const extension = mensagem.nome_arquivo?.split('.').pop() || 'pdf';

    // Enviar documento
    const payload: any = {
      phone: numeroDestinatario,
      document: mensagem.url_midia,
      extension: extension,
    };

    if (mensagem.nome_arquivo) {
      payload.fileName = mensagem.nome_arquivo;
    }
    ...
    const wapiUrl = `https://api.w-api.app/v1/message/send-document?instanceId=${conta.instance_id_wapi}`;
    ...
    console.log('✅ Documento enviado:', wapiData);
