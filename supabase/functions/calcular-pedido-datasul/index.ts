...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Dividindo ${itens.length} itens em ${totalLotes} lote(s) de ${TAMANHO_LOTE}`);

    // Marcar como processando e retornar imediato
    ...
    // Processar todos os lotes em background
    const processarTodosLotes = async () => {
      ...
      for (let i = 0; i < totalLotes; i++) {
        ...
        for (let tentativa = 0; tentativa < 3 && !sucesso; tentativa++) {
          try {
            ...
            console.error(`Lote ${numeroLote} tentativa ${tentativa+1} falhou:`, error);
          }
        }
        ...
        if (sucesso) {
          ...
          console.error(`Lote ${numeroLote} falhou após 3 tentativas`);
        }
      }
      ...
      console.log(`Processamento concluído: ${resultados.length}/${totalLotes} lotes com sucesso`);
    };

    // Iniciar background processing
    processarTodosLotes().catch(err => console.error("Erro no processamento:", err));
    ...
    return new Response(
      JSON.stringify({
        ...
        resumo: {
          ...
          lotes_em_background: totalLotes,
          tempo_resposta_ms: tempoResposta,
        },
        processamento_completo: false,
        mensagem: `Cálculo iniciado! ${totalLotes} lote(s) de ${TAMANHO_LOTE} itens sendo processados em background.`,
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
