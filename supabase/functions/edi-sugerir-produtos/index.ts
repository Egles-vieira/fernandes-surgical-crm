    ...
    const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Você é um especialista em análise de produtos médico-hospitalares. Retorne apenas JSON válido.",
          },
...
serve(async (req) => {
  ...
  try {
    ...
    if (!produtos || produtos.length === 0) {
      return new Response(JSON.stringify({ sugestoes: [], total_produtos_analisados: 0, metodo: "nenhum" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    console.log(
      `📊 Token matching: ${candidatosPorToken.length} candidatos (melhor: ${candidatosPorToken[0]?.score || 0})`,
    );
    ...
    if (analiseSemantica.length > 0) {
      ...
      for (let i = 0; i < Math.min(candidatosPorToken.length, limite); i++) {
        ...
        if (ajusteML !== 0) {
          ...
          console.log(
            `  🎯 Produto ${candidato.produto.referencia_interna}: score ${scoreFinal - ajusteML} → ${scoreFinal} (ajuste: ${ajusteML > 0 ? "+" : ""}${ajusteML})`,
          );
        }
      ...
      for (const candidato of candidatosPorToken.slice(0, limite)) {
        ...
        if (ajusteML !== 0) {
          scoreFinal = Math.max(0, Math.min(100, scoreFinal + ajusteML));
          console.log(
            `  🎯 Produto ${candidato.produto.referencia_interna}: score ${candidato.score} → ${scoreFinal} (ajuste: ${ajusteML > 0 ? "+" : ""}${ajusteML})`,
          );
        }
