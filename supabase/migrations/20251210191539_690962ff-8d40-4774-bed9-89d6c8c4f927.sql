-- Corrigir coluna previsao_fechamento para data_fechamento_prevista
...
BEGIN
  ...
  WHERE 
    (vct.etapa_pipeline = 'leads' AND vct.rn <= COALESCE((p_limites_por_etapa->>'leads')::int, 20))
    OR (vct.etapa_pipeline = 'contato' AND vct.rn <= COALESCE((p_limites_por_etapa->>'contato')::int, 20))
