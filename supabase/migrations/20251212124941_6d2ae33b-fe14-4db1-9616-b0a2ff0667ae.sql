...
DROP FUNCTION IF EXISTS public.get_vendas_pipeline_paginado(JSONB);
DROP FUNCTION IF EXISTS public.get_vendas_pipeline_paginado(JSONB, INT);

-- Recriar com a versão original que funcionava
...
BEGIN
  ...
  WITH vendas_com_totais AS (
    SELECT 
      ...
      v.probabilidade,
      v.created_at,
      v.data_fechamento_prevista::TIMESTAMPTZ as data_fechamento_prevista,
