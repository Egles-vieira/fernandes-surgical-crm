-- Função para criar notificações de vendas
...
BEGIN
  ...
  SELECT COALESCE(primeiro_nome || ' ' || sobrenome, primeiro_nome, email)
...
-- Criar trigger
DROP TRIGGER IF EXISTS trigger_notificar_vendas ON vendas;
CREATE TRIGGER trigger_notificar_vendas
  AFTER INSERT OR UPDATE ON vendas
  FOR EACH ROW
  EXECUTE FUNCTION notificar_mudancas_venda();