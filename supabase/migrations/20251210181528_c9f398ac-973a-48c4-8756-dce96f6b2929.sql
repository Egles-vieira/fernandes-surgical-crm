-- Drop da função existente para recriar com correção
DROP FUNCTION IF EXISTS public.get_vendas_pipeline_paginado(jsonb, integer);

-- Recriar função com cast ::text corrigido em todas as 3 ocorrências
...
BEGIN
  ...
  SELECT EXISTS (
    ...
    WHERE ur.user_id = v_user_id AND r.nivel <= 2
  ) INTO v_is_admin;
  ...
  SELECT EXISTS (
    SELECT 1 FROM user_roles ur
    JOIN roles r ON ur.role_id = r.id
    WHERE ur.user_id = v_user_id AND r.nivel <= 3
  ) INTO v_is_manager;
