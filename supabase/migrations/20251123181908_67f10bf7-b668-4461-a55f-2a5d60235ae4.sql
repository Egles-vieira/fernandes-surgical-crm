-- Corrigir tipo de quantidade_em_maos de integer para numeric
-- Necessário fazer DROP primeiro pois o PostgreSQL não permite alterar tipo de retorno

DROP FUNCTION IF EXISTS match_produtos_hibrido(text, vector, float, int);

CREATE FUNCTION match_produtos_hibrido(
  query_text text,
...
RETURNS TABLE (
  ...
  narrativa text,
  preco_venda numeric,
  quantidade_em_maos numeric,  -- ✅ Corrigido de integer para numeric
  similarity float,
  match_type text
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Filtrar palavras com menos de 3 caracteres
  words := ARRAY(
    SELECT w 
    FROM unnest(words) w 
    WHERE length(w) >= 3
  ...
  combined AS (
    ...
    SELECT DISTINCT ON (tm.product_id)
      tm.product_id as id,
      tm.referencia_interna,
      tm.nome,
      tm.narrativa,
      tm.preco_venda, 
      tm.quantidade_em_maos,
      tm.similarity,
      tm.match_type
    FROM text_matches tm
    WHERE tm.word_matches >= min_word_matches
    ...
    SELECT 
      sm.product_id as id,
      sm.referencia_interna,
      sm.nome,
      sm.narrativa,
      sm.preco_venda, 
      sm.quantidade_em_maos,
      sm.similarity,
      sm.match_type
