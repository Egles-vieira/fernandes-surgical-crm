-- Atualizar trigger para incluir followup_cliente
CREATE OR REPLACE FUNCTION public.notificar_mudancas_venda()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.etapa_pipeline IS DISTINCT FROM NEW.etapa_pipeline THEN
      ...
      v_descricao := 'Venda #' || NEW.numero_venda || ' movida para ' || 
        CASE NEW.etapa_pipeline
          ...
          WHEN 'followup_cliente' THEN 'Follow-up Cliente'
          WHEN 'fechamento' THEN 'Fechamento'
          WHEN 'ganho' THEN 'Ganho'
          WHEN 'perdido' THEN 'Perdido'
          ELSE NEW.etapa_pipeline::TEXT
        END;
    ...
    IF OLD.status IS DISTINCT FROM NEW.status THEN
      ...
      v_titulo := 'Status da venda alterado';
      v_descricao := 'Venda #' || NEW.numero_venda || ' teve status alterado para ' || NEW.status;
      v_tipo := 'venda_status_alterado';
    ...
    -- Venda aprovada
    IF OLD.aprovado_em IS NULL AND NEW.aprovado_em IS NOT NULL THEN
