-- Corrigir search_path das funções criadas para Dashboard de Metas
DROP FUNCTION IF EXISTS get_kpis_gerais_periodo(timestamp with time zone, timestamp with time zone);
DROP FUNCTION IF EXISTS get_pacing_semanal(timestamp with time zone, timestamp with time zone);
DROP FUNCTION IF EXISTS get_funil_vendas(timestamp with time zone, timestamp with time zone);

-- Recriar com search_path correto
