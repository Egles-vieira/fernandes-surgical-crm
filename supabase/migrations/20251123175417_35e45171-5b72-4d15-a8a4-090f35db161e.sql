-- Drop todas as versões existentes da função match_produtos_hibrido
DROP FUNCTION IF EXISTS match_produtos_hibrido(text, vector, float, int);
DROP FUNCTION IF EXISTS match_produtos_hibrido(vector, text, float, int);

-- Criar versão definitiva única com busca por palavras individuais
...
BEGIN
  -- Dividir o texto em palavras (excluir palavras < 3 chars)
  words := regexp_split_to_array(lower(trim(query_text)), '\s+');
  words := array_remove(words, '');
  words := ARRAY(SELECT unnest(words) WHERE length(unnest) >= 3);
