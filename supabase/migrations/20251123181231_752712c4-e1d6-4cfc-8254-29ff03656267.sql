-- Corrigir tipo de retorno da função match_produtos_hibrido com casts explícitos
...
BEGIN
  ...
  IF array_length(words, 1) IS NULL OR array_length(words, 1) = 0 THEN
    ...
    SELECT 
      p.id,
      p.referencia_interna::text,  -- Cast explícito #1
      p.nome,
      p.narrativa,
      p.preco_venda,
  ...
  WITH text_matches AS (
    SELECT 
      p.id as product_id,
      p.referencia_interna::text,  -- Cast explícito #2
      p.nome,
      p.narrativa,
      p.preco_venda,
      p.quantidade_em_maos,
  ...
  semantic_matches AS (
    SELECT 
      p.id as product_id,
      p.referencia_interna::text,  -- Cast explícito #3
      p.nome,
      p.narrativa,
      p.preco_venda,
      p.quantidade_em_maos,
  ...
  SELECT DISTINCT ON (c.id)
    c.id,
    c.referencia_interna::text,  -- Cast explícito #4
    c.nome,
    c.narrativa,
    c.preco_venda,
