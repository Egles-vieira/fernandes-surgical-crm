...
export function useGerarLinkPropostaOportunidade(oportunidadeId: string | null) {
  const queryClient = useQueryClient();
  const [publicUrl, setPublicUrl] = useState<string | null>(null);

  const gerarLinkMutation = useMutation({
    mutationFn: async (config?: GerarLinkConfig) => {
      if (!oportunidadeId) throw new Error("ID da oportunidade não fornecido");

      const { data, error } = await supabase.functions.invoke("gerar-link-proposta-oportunidade", {
        body: { oportunidadeId, config }
      });
    ...
    onSuccess: (token) => {
      // Usar rota específica para oportunidades
      const url = `${window.location.origin}/proposal-oportunidade/${token}`;
      setPublicUrl(url);
