...
const AgenteVendasToggle = ({
  contaId,
  agenteAtivo
}: AgenteVendasToggleProps) => {
  const {
    toast
  } = useToast();
  ...
  const toggleMutation = useMutation({
    mutationFn: async (novoStatus: boolean) => {
      const {
        error
      } = await supabase.from('whatsapp_contas').update({
        agente_vendas_ativo: novoStatus
      }).eq('id', contaId);
    ...
    onSuccess: (_, novoStatus) => {
      ...
      toast({
        ...
        description: novoStatus ? 'O agente de vendas responderá automaticamente às mensagens' : 'O agente de vendas foi desativado'
      });
      setAtivo(novoStatus);
    },
    onError: error => {
  ...
  return <Card className="border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-base flex items-center gap-2">
                Agente de Vendas IA
                {ativo && <Badge variant="default" className="gap-1">
          ...
          <div className="flex items-center gap-2">
            <Switch checked={ativo} onCheckedChange={checked => toggleMutation.mutate(checked)} disabled={toggleMutation.isPending} />
