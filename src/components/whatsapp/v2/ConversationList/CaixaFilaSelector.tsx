...
const caixaOptions: CaixaOption[] = [{
  ...
  icon: Inbox,
  contadorKey: 'meus_atendimentos'
}, {
  value: 'nao_lidas',
  label: 'Mensagens não Lidas',
  icon: MessageCircle,
  contadorKey: 'nao_lidas'
}, {
  value: 'fila_espera',
  label: 'Fila de Espera',
  icon: Clock,
  contadorKey: 'fila_espera'
}, {
  value: 'chatbot',
  label: 'ChatBot',
  icon: Bot,
...
const caixasSecundarias: CaixaOption[] = [{
  ...
  value: 'todos_nao_lidas',
  label: 'Todos (não lidas)',
  icon: MessageCircle,
  contadorKey: 'todos_nao_lidas'
}];
export function CaixaFilaSelector({
  ...
  return <DropdownMenu>
      ...
      <DropdownMenuContent align="start" className="w-56">
        ...
        return <DropdownMenuItem key={option.value} onClick={() => onChange(option.value)} className={cn("flex items-center justify-between gap-2 cursor-pointer", value === option.value && "bg-primary/10")}>
              <div className="flex items-center gap-2 min-w-0">
                <Icon className="h-4 w-4 shrink-0 text-muted-foreground" />
                <span className="truncate">{option.label}</span>
              </div>
              {!isLoading && contador > 0 && <Badge variant={option.value === 'nao_lidas' || option.value === 'fila_espera' ? 'destructive' : 'secondary'} className="h-5 min-w-5 px-1.5 text-xs font-bold">
        ...
        return <DropdownMenuItem key={option.value} onClick={() => onChange(option.value)} className={cn("flex items-center justify-between gap-2 cursor-pointer", value === option.value && "bg-primary/10")}>
              ...
              {!isLoading && contador > 0 && <Badge variant="outline" className="h-5 min-w-5 px-1.5 text-xs font-medium">
                  {contador > 9999 ? '9999+' : contador}
                </Badge>}
            </DropdownMenuItem>;
      })}
