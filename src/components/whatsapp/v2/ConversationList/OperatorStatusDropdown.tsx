  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className="h-8 gap-1.5 text-xs font-medium" disabled={isChanging}>
          <span className={cn("w-2 h-2 rounded-full shrink-0 animate-pulse", selectedOption.bgColor)} />
        ...
        {statusOptions.map((option) => {
          ...
          return (
            <DropdownMenuItem
              ...
              disabled={isChanging}
              className={cn("flex items-center gap-2 cursor-pointer", status === option.value && "bg-primary/10")}
            >
              <span className={cn("w-2.5 h-2.5 rounded-full shrink-0", option.bgColor)} />
