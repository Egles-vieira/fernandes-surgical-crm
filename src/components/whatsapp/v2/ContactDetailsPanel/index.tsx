...
import { X, Phone, Mail, Building2, Tag, FileText, Clock, User, ExternalLink, Edit, Wallet, Check, Loader2 } from 'lucide-react';
...
export function ContactDetailsPanel({
  ...
  const {
    ...
    isRemovendoContato
  } = useWhatsAppCarteiras();

  // Fetch additional contact data
  const {
    data: contatoCompleto
  } = useQuery({
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('whatsapp_contatos').select(`
          *,
          contatos (
            id,
            primeiro_nome,
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await (supabase as any).from('whatsapp_carteiras_contatos').select(`
          id,
          carteira_id,
        ...
        `).eq('whatsapp_contato_id', contato.id).maybeSingle();
      if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('whatsapp_mensagens').select('id, direcao, criado_em').eq('conversa_id', conversaId);
      if (error) throw error;
  ...
  if (!contato) {
    return <div className="h-full flex items-center justify-center bg-card border-l">
  ...
  return <div className="h-full flex flex-col bg-card border-l">
      {/* Header */}
      ...
      <ScrollArea className="flex-1">
        <div className="p-4 space-y-4">
          ...
          <Accordion type="multiple" defaultValue={['crm', 'carteira', 'stats']} className="space-y-2">
            {/* CRM Link */}
            {linkedContact ? <AccordionItem value="crm" className="border rounded-lg px-4">
                ...
                <AccordionContent className="pb-3 space-y-2">
                  ...
                  {linkedContact.email && <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Mail className="h-3 w-3" />
                      {linkedContact.email}
                    </div>}
                  {linkedContact.cargo && <Badge variant="outline" className="text-xs">
                      {linkedContact.cargo}
                    </Badge>}
                </AccordionContent>
              </AccordionItem> : <AccordionItem value="crm" className="border border-dashed rounded-lg px-4">
                <AccordionTrigger className="py-3 hover:no-underline">
                  <span className="text-xs font-medium flex items-center gap-2">
                    <User className="h-3.5 w-3.5" />
                    Contato no CRM
            ...
            {/* Client Info */}
            {linkedClient && <AccordionItem value="empresa" className="border rounded-lg px-4">
                <AccordionTrigger className="py-3 hover:no-underline">
                  <span className="text-xs font-medium flex items-center gap-2">
                ...
                <AccordionContent className="pb-3 space-y-1">
                  ...
                  {linkedClient.cgc && <p className="text-xs text-muted-foreground">
            ...
            <AccordionItem value="carteira" className="border rounded-lg px-4">
              ...
              <AccordionContent className="pb-3">
                {carteiraAtual ? <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full" style={{
                    backgroundColor: carteiraAtual.carteira?.cor || '#3b82f6'
                  }} />
                      <span className="text-sm font-medium">{carteiraAtual.carteira?.nome}</span>
                    </div>
                    {carteiraAtual.carteira?.operador?.nome_completo && <p className="text-xs text-muted-foreground">
                        Operador: {carteiraAtual.carteira.operador.nome_completo}
                      </p>}
                    <Button variant="outline" size="sm" className="w-full text-destructive hover:text-destructive" onClick={handleRemoverCarteira} disabled={isRemovendoContato}>
                      {isRemovendoContato ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <X className="h-3.5 w-3.5 mr-1.5" />}
                      Remover da Carteira
                    </Button>
                    ...
                    <Select value={selectedCarteira} onValueChange={setSelectedCarteira}>
                      ...
                      <SelectContent className="bg-popover z-50">
                        {carteiras?.map(carteira => <SelectItem key={carteira.id} value={carteira.id}>
                            <div className="flex items-center gap-2">
                              <div className="w-2 h-2 rounded-full" style={{
                    ...
                    <Button variant="default" size="sm" className="w-full" onClick={handleCarteirizar} disabled={!selectedCarteira || isAdicionandoContato}>
                      {isAdicionandoContato ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Check className="h-3.5 w-3.5 mr-1.5" />}
                      Carteirizar
                    </Button>
                  </div>}
              </AccordionContent>
            ...
            {/* Estatísticas */}
            {stats && <AccordionItem value="stats" className="border rounded-lg px-4">
                <AccordionTrigger className="py-3 hover:no-underline">
                  <span className="text-xs font-medium flex items-center gap-2">
                ...
                <AccordionContent className="pb-3">
                  ...
                  {stats.firstMessage && <p className="text-[10px] text-muted-foreground text-center mt-2">
                      Primeira mensagem: {format(new Date(stats.firstMessage), "dd/MM/yyyy 'às' HH:mm", {
                  locale: ptBR
                })}
                    </p>}
                </AccordionContent>
