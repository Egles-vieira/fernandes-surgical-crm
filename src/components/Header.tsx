  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("perfis_usuario").select("*").eq("id", user.id).single();
        ...
        <div className="flex items-center gap-4 ml-auto">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="flex items-center gap-3 hover:bg-muted/50 transition-all h-auto py-2 px-3 rounded-lg">
                <Avatar className="h-10 w-10 border-2 border-primary/20 shadow-sm">
                  {perfil?.foto_perfil_url ? <AvatarImage src={perfil.foto_perfil_url} alt={`${perfil.primeiro_nome} ${perfil.sobrenome}` || "Usuário"} /> : null}
                ...
                <div className="text-sm leading-tight text-left">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    {roles && roles.length > 0 && <Badge variant="secondary" className="text-[10px] px-1.5 py-0 h-4">
                        {roles[0]}
                      </Badge>}
                  </div>
                </div>
            ...
            <DropdownMenuContent align="end" className="w-72 border-0 shadow-elegant bg-card">
              <DropdownMenuLabel className="pb-3">
                ...
                {roles && roles.length > 0 && <div className="flex flex-wrap gap-1.5 mt-3">
                    {roles.map(role => <Badge key={role} variant="secondary" className="text-xs px-2 py-0.5">
                        {role}
                      </Badge>)}
                  </div>}
              </DropdownMenuLabel>
              ...
              {roles?.includes("admin") && <DropdownMenuItem onClick={() => navigate("/usuarios")} className="cursor-pointer py-2.5">
