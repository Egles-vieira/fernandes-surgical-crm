...
export function ItemCotacaoTable({
  ...
  return <>
          ...
          <TableBody>
            ...
            return <>
                  ...
                  <TableRow key={item.id} className="hover:bg-muted/50">
                    ...
                    <TableCell>
                      <Input type="number" value={data?.quantidade || 0} onChange={e => updateItemField(item.id, "quantidade", Number(e.target.value))} className="w-20 h-8" min={0} />
                    </TableCell>
                    <TableCell>
                      <Input type="number" value={data?.precoUnitario || 0} onChange={e => updateItemField(item.id, "precoUnitario", Number(e.target.value))} className="w-24 h-8" min={0} step={0.01} />
                    </TableCell>
                    <TableCell>
                      <Input type="number" value={data?.desconto || 0} onChange={e => updateItemField(item.id, "desconto", Number(e.target.value))} className="w-20 h-8" min={0} max={100} step={0.01} />
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        <Button variant="outline" size="sm" onClick={() => handleAnalisarIA(item)} disabled={data?.isLoading}>
                          <Sparkles className="h-3 w-3" />
                        </Button>
                        <Button size="sm" onClick={() => handleSalvar(item)} disabled={data?.isLoading || !data?.produtoVinculado}>
                  ...
                  {isExpanded && mappings.length > 0 && mappings.map(mapping => <TableRow key={mapping.id} className="bg-muted/20 hover:bg-muted/40 cursor-pointer" onClick={() => handleSelectPreviousMapping(item.id, mapping)}>
                      <TableCell></TableCell>
                      <TableCell></TableCell>
      ...
      <SugestoesIADialog open={sugestoesDialogAberto} onOpenChange={setSugestoesDialogAberto} sugestoes={sugestoesIA} onSelecionar={handleSelecionarSugestao} onBuscarManual={() => {
