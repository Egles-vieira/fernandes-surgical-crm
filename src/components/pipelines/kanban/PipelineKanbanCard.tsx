          ...
          <div className="p-3 bg-secondary-foreground overflow-hidden min-w-0">
            ...
            <div className="flex items-start justify-between gap-2 mb-2 overflow-hidden">
              ...
              <h4 className="font-medium text-sm text-foreground truncate" title={oportunidade.nome}>
                  {oportunidade.nome.length > 25 ? `${oportunidade.nome.substring(0, 25)}...` : oportunidade.nome}
                </h4>
            ...
            {/* Valor em destaque */}
            {oportunidade.valor != null && oportunidade.valor > 0 && <div className="flex items-center gap-1.5 mb-2 p-1.5 rounded bg-primary/10 overflow-hidden">
                <DollarSign className="h-3.5 w-3.5 text-primary shrink-0" />
                <span className="text-sm font-bold text-primary truncate">
                  {formatCurrency(oportunidade.valor)}
                </span>
                {oportunidade.valorPonderado != null && <span className="text-xs text-muted-foreground ml-auto shrink-0">
            ...
            {Object.keys(oportunidade.camposKanban).length > 0 && <div className="mt-2 pt-2 border-t border-border/50 space-y-1 overflow-hidden">
                {Object.entries(oportunidade.camposKanban).slice(0, 3).map(([key, value]) => <div key={key} className="flex items-center justify-between text-xs gap-2 min-w-0 overflow-hidden">
