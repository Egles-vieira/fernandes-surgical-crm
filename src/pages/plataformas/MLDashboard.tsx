...
export default function MLDashboard() {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      const feedbacksRecentes = feedbackData?.filter(f => 
        new Date(f.criado_em) >= dataLimite && f.foi_aceito
      ).length || 0;

      // 2. Ajustes ativos de ML
      const { data: ajustesData, count: totalAjustes } = await supabase
      ...
      const { data: topProdutosData } = await supabase
        ...
        `)
        .eq('ativo', true)
        .order('ultima_utilizacao_em', { ascending: false })
        .limit(10);
      ...
      const produtos = Array.from(produtosMap.values())
        .map(p => ({
          ...p,
          taxa_sucesso: p.total_aceito + p.total_rejeitado > 0
            ? (p.total_aceito / (p.total_aceito + p.total_rejeitado)) * 100
            : 0
        }))
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Acompanhe o aprendizado da IA com feedback dos vendedores
          </p>
        </div>
      </div>
